...
export class App {
    ...
    private constructor() {
        ...
        this.matrixClient.on(sdk.RoomEvent.Timeline, (event, room, toStartOfTimeline) => {
            // only listen to messages in the given room which were not sent by this bot
            if (event.getType() !== "m.room.message" || event.event.room_id != this.config.getRoomId() || event.event.sender == this.config.getUserId()) {
                return;
            }
    ...
    private processMessage(message: string): void {
        const command: Command | undefined = this.commandList.find(e => e.command === message)
    ...
    public sendMessage(message: string): void {
        ...
        app.matrixClient.sendEvent(app.config.getRoomId(), "m.room.message", content, "");
    }
